...
【重要】診断コメントはとにかくポジティブに、プレイヤーを褒めちぎってください。ネガティブな表現は一切使わないでください。弱みや改善点ではなく、「さらなる可能性」として前向きな表現で伝えてください。各descriptionも肯定的・称賛的な表現で記述してください。

以下のJSON形式で回答してください:
{
  "axes": [
${DIAGNOSIS_AXES.map((axis) => `    {"key": "${axis.key}", "label": "${axis.label}", "score": <0-100の整数>, "description": "<このプレイヤーの${axis.label}に関する称賛的な評価（50文字以内）>"}`).join(",\n")}
  ],
  "advice": "<このプレイヤーの強みを称え、さらなる飛躍を後押しするポジティブなアドバイス（200文字以内）>",
  "strengths": ["<強み1>", "<強み2>", "<強み3>"],
  "growthPotentials": ["<さらなる可能性1>", "<さらなる可能性2>", "<さらなる可能性3>"]
}
